...
shade_colors=brighten(line_colors,.5);
...
drought_severity=ecoregion_drought_severity;        
ploty_temp=ecoregion_drought_resistance;

s1=scatter(drought_severity,ploty_temp,scatter_size,line_colors,'filled');  
...
[b,~,~,~,stats] = regress(ploty_temp(~idx3)',[drought_severity(~idx3)',ones(length(drought_severity(~idx3)),1)]);
% change_slope=b(1,1);            
change_p=stats(3);
...
[yfit,~]=polyconf(p,drought_severity(~idx3),s);

            
plot(drought_severity(~idx3),yfit,'-','Color',line_colors,'Linewidth', line_width);        
hold on  
xfit = nanmin(drought_severity):0.1:nanmax(drought_severity);
% yfit = polyval(p,xfit);
hold on
[Y,DELTA] = polyconf(p,xfit,s,'alpha',0.05,'predopt','curve');
h=fill([xfit fliplr(xfit)], [Y+DELTA fliplr(Y-DELTA)], shade_colors,'edgecolor','none');
set(h,'facealpha',.2)   


line([1.2 2], [-1 -1],'LineStyle','--','color','k') 
...
if change_p<=0.05
    text(xlim1(1)+(xlim1(2)-xlim1(1))*0.5,-0.4,['y = ' num2str(roundn(p(1),-2)) '*x - ' num2str(abs(roundn(p(2),-2)))],'color',line_colors,'FontWeight','bold') 
    text(xlim1(1)+(xlim1(2)-xlim1(1))*0.5,-0.52,'{\itP} < 0.05','fontweight','bold','color',line_colors)  
else
    text(xlim1(1)+(xlim1(2)-xlim1(1))*0.5,-0.4,['y = ' num2str(roundn(p(1),-2)) '*x - ' num2str(abs(roundn(p(2),-2)))],'color',line_colors,'FontWeight','bold') 
    text(xlim1(1)+(xlim1(2)-xlim1(1))*0.5,-0.52,'{\itP} > 0.05','fontweight','bold','color',line_colors)      
end
...
ploty_temp=ecoregion_drought_resilience;
...
hold on
xlim1=xlim;
line([xlim1], [0 0],'LineStyle','--','color','k') 
hold on
ylim1=ylim;
line([prctile(ecoregion_drought_severity(:),75) prctile(ecoregion_drought_severity(:),75)], [ylim1],'LineStyle','--','color','k')
...
line([1 2.4], [-1 -1],'LineStyle','--','color','k') 
...
all_slope_asia=[all_slope_asia;change_slope];
...
line([0.8 5.5], [-1 -1],'LineStyle','--','color','k') 


set(gca,'ylim',[-1.1 -0.3])
set(gca,'ytick',[-1:0.2:-0.4])
set(gca,'yticklabel',{})
set(gca,'xlim',[0.8 5.5])
set(gca,'xtick',1:1:5)
...
%%
tightfig
% print(gcf,'-dtiff','-r300',strcat('./PNAS_R2_Fig3.tif'))
